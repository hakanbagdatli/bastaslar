...
namespace WebSite
{
    public partial class Records : System.Web.UI.Page
    {
        ...
        protected void GeneralList(int CatID, string Sorting, int RowCount)
        {
            ...
            var _idList = Select.MultipleCategoryID(CatID).Split(',').Select(int.Parse).ToList();
            dataList = StaticList.Records.Where(x => (_idList.Contains(x.CatID)) && (x.Approved == 1)).ToList();
