...
namespace WebSite
{
    public partial class ProductDetail : System.Web.UI.Page
    {
        ...
        public List<Entities.GeneralPlans> planList;
        public List<Entities.GeneralPlans> planDetail;
            ...
            if (dataList.Count > 0)
            {
                foreach (var item in dataList)
                {
                    ...
                    if (Request["property"] == null)
                    {
                        ...
                        //---------------------------------------------------------
                        DetailGallery(item.id, 0);
                        DetailFeatures(item.id);
                        //DetailFiles(item.id);
                        DetailVideos(item.id);
                        DetailPlans(item.id);
                    }
                    else
                    {
                        if (Convert.ToBoolean(Request["property"]))
                        {
                            if (Request["v"] != null)
                            {
                                Dictionary<string, object> _dic = new Dictionary<string, object>();
                                _dic.Add("propertyID", Request["v"].ToString());
                                planDetail = Bll.GeneralPlans.Select(0, " AND PropertyID=@propertyID AND Approved=1", "", _parmsVals: _dic);
                                if (planDetail.Count > 0)
                                {
                                    foreach (var plan in planDetail)
                                    {
                                        isProperty = true;
                                        PageProperties(item.id, item.CatID,
                                            Handler.SetText(plan.Title, ""),
                                            Handler.SetText(plan.Title, ""),
                                            Handler.SetText(plan.Title, ""),
                                            Handler.SetText(item.Description, item._Description),
                                            Handler.SetText(item.Keywords, item._Keywords));
                                        DetailGallery(item.id, plan.id);
                                        DetailFeatures(item.id);
                                        //DetailFiles(item.id);
                                        DetailVideos(item.id);
                                    }
        ...
        protected void PageProperties(int RecordID, int CatID, string Title, string MetaTitle, string H1Title, string Description, string Keywords)
        {
            ...
            //---------------------------------------------
            lblTitle.Text = MetaTitle;
            Breadcrumb.MultipleTreeDetailWhereAmI(CatID, Title, MetaTitle, lblBreadcrumb, this.Page);
            #endregion
        }
        //--------------------------------------------------------- description, meta tag vs.

        protected void DetailGallery(int RecordID, int PlanID)
        {
            if (PlanID == 0)
                photoList = Bll.GeneralPhotos.Select(0, filter: " AND TypeID=0 AND LangID in(0," + Feature.ActiveLanguage + ")  AND CatID=" + RecordID + " AND Approved=1 AND Image<>''", sorting: " Sorting ASC, id ASC");
            else
                photoList = Bll.GeneralPhotos.Select(0, filter: " AND TypeID=0 AND LangID in(0," + Feature.ActiveLanguage + ")  AND PropertyID in(0," + PlanID +") AND CatID=" + RecordID + " AND Approved=1 AND Image<>''", sorting: " TurID DESC, Sorting ASC, id ASC");
        ...
        protected void DetailVideos(int RecordID)
        {
        ...
        protected void DetailPlans(int RecordID)
        {
            planList = Bll.GeneralPlans.Select(0, filter: " AND LangID in(0," + Feature.ActiveLanguage + ")  AND CatID=" + RecordID + " AND Approved=1", sorting: " Sorting ASC, id DESC");
            if (planList.Count > 0)
